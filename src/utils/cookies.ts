...
export const remove = function(name) {
    if (typeof (Storage) !== 'undefined') {
        ...
        const expires = 'expires=' + d.toUTCString();
        document.cookie = 'antifraud-user-auth-token' + '=' + undefined + ';' + expires + ';path=/';
    }
};
...
export const get = function(cname) {
    if (cname === 'antifraud-user-auth-token' && !prepareGet('antifraud-user-auth-token')) {
        return null;
    }
    if (cname === 'antifraud-user-auth-token') {
...
export const getToken = function() {
    if (!prepareGet('antifraud-user-auth-token')) {
        return null;
    }
    return EncryptionService.decrypt(prepareGet('antifraud-user-auth-token'));
